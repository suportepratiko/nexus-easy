# python
from safirionapi.api import Safirionapi
import safirionapi.constants as OP_code
import safirionapi.country_id as Country
import threading
import time
import json
import logging
...
from safirionapi.expiration import get_expiration_time, get_remaning_time
from safirionapi.version_control import api_version
...
class Safirion:
    ...
    def connect(self, sms_code=None):
        ...
        # Update the host — lido do .env (SAFIRION_HOST) ou fallback padrão
        import os
    ...
    def __get_digital_open(self):
        # for digital options
        result = self.get_digital_underlying_list_data()
        
        # Verificação de segurança: garantir que os dados foram recebidos corretamente
        if result is None:
            logging.warning("get_digital_underlying_list_data retornou None - dados digitais não disponíveis")
            return
        
        # Verificar se o resultado tem a chave 'underlying'
        if "underlying" not in result:
            logging.warning(f"Resposta inesperada de get_digital_underlying_list_data - chave 'underlying' não encontrada. Resposta: {type(result)}")
            return
        
        digital_data = result["underlying"]
    ...
    def __get_other_open(self):
        ...
        for instruments_type in instrument_list:
            ...
            # Verificação de segurança: garantir que os dados foram recebidos corretamente
            if result is None:
                logging.warning(f"get_instruments({instruments_type}) retornou None - dados não disponíveis")
                continue
            
            # Verificar se o resultado tem a chave 'instruments'
            if "instruments" not in result:
                logging.warning(f"Resposta inesperada de get_instruments({instruments_type}) - chave 'instruments' não encontrada")
                continue
            
            ins_data = result["instruments"]
    ...
    def get_available_turbo_actives(self):
        """Retorna lista de nomes de ativos turbo disponíveis (não suspensos)."""
        try:
            data = self.get_all_init_v2()
            if not data:
                return list(OP_code.ACTIVES.keys())[:10]
            # init v2 pode vir como msg direto ou sob "result"
            turbo = (data.get("result") or data).get("turbo") or data.get("turbo")
            if not turbo or "actives" not in turbo:
                return list(OP_code.ACTIVES.keys())[:10]
            out = []
            for actives_id, active in turbo["actives"].items():
                if active.get("enabled") and not active.get("is_suspended", True):
                    raw_name = active.get("name") or ""
                    name = str(raw_name).split(".")[-1] if raw_name else ""
                    if name and name in OP_code.ACTIVES:
                        out.append(name)
            return out if out else list(OP_code.ACTIVES.keys())[:10]
        except Exception as e:
            logging.warning("get_available_turbo_actives: %s", e)
            return list(OP_code.ACTIVES.keys())[:10]

    def get_available_binary_actives(self):
        """Retorna lista de nomes de ativos binary (não turbo) disponíveis."""
        try:
            data = self.get_all_init_v2()
            if not data:
                return []
            binary = (data.get("result") or data).get("binary") or data.get("binary")
            if not binary or "actives" not in binary:
                return []
            out = []
            for actives_id, active in binary["actives"].items():
                if active.get("enabled") and not active.get("is_suspended", True):
                    raw_name = active.get("name") or ""
                    name = str(raw_name).split(".")[-1] if "." in str(raw_name) else str(raw_name).strip()
                    if name and name in OP_code.ACTIVES:
                        out.append(name)
            return out
        except Exception as e:
            logging.warning("get_available_binary_actives: %s", e)
            return []

    def get_available_digital_actives(self):
        """Retorna lista de nomes de ativos digitais disponíveis (mercado aberto no momento)."""
        try:
            result = self.get_digital_underlying_list_data()
            if not result or "underlying" not in result:
                return []
            out = []
            for digital in result["underlying"]:
                name = digital.get("underlying") or ""
                if not name or name not in OP_code.ACTIVES:
                    continue
                schedule = digital.get("schedule") or []
                is_open = False
                now = time.time()
                for schedule_time in schedule:
                    start = schedule_time.get("open") or 0
                    end = schedule_time.get("close") or 0
                    if start < now < end:
                        is_open = True
                        break
                if is_open:
            ...
            logging.warning("get_available_digital_actives: %s", e)
            return []

    def _filter_actives_by_market(self, names, allow_open, allow_otc):
        """Filtra lista de nomes de ativos por mercado: aberto (sem -OTC) e/ou OTC (com -OTC)."""
        if not names:
            return []
        out = []
        for name in names:
            is_otc = name.endswith("-OTC")
            if is_otc and allow_otc:
                out.append(name)
            elif not is_otc and allow_open:
                out.append(name)
        return out

    def get_available_actives_filtered(self, allow_digital=True, allow_binary=True, allow_open=True, allow_otc=True):
        """
        Retorna lista de ativos disponíveis conforme modalidade e mercado.
        Cada item: {"name": str, "type": "binary" | "digital"}.
        """
        result = []
        if allow_binary:
            turbo = self.get_available_turbo_actives()
            binary = self.get_available_binary_actives()
            all_binary_names = list(dict.fromkeys(turbo + binary))
            filtered = self._filter_actives_by_market(all_binary_names, allow_open, allow_otc)
            for name in filtered:
                result.append({"name": name, "type": "binary"})
        if allow_digital:
            digital_names = self.get_available_digital_actives()
            filtered = self._filter_actives_by_market(digital_names, allow_open, allow_otc)
            for name in filtered:
                result.append({"name": name, "type": "digital"})
        if not result:
            result = [{"name": "EURUSD", "type": "binary"}]
        return result
...
##############################################################################################
    ...
    def get_binary_option_result(self, order_id, timeout_sec=95):
        """Aguarda option-closed para order_id e retorna (is_win: bool, profit: float). Evita usar diferença de saldo."""
        start = time.time()
        oid_int = int(order_id) if order_id is not None else None
        oid_str = str(order_id) if order_id is not None else None
        while time.time() - start < timeout_sec:
            key = oid_int if oid_int in self.api.order_binary else (oid_str if oid_str in self.api.order_binary else None)
            if key is not None:
                your_order = self.api.order_binary.pop(key, None)
                if your_order is None:
                    time.sleep(0.5)
                    continue
                ...
                if win in ("win", "won"):
                    ...
                    return True, round(profit_val, 2)
                if win in ("loose", "loss", "lose"):
                    return False, round(-sum_inv, 2)
                return False, 0.0  # equal
            time.sleep(0.8)
        logging.warning("get_binary_option_result: timeout order_id=%s", order_id)
        return None, 0.0
...
# __________________for Digital___________________
    ...
    def poll_digital_result(self, order_id, timeout_sec=68):
        """
        Consulta o histórico/evento da corretora (position-changed) para obter resultado da opção digital.
        Retorna (profit,) assim que a posição fechar, ou None após timeout. Poll a cada 2s.
        """
        start = time.time()
        while time.time() - start < timeout_sec:
            try:
                data = self.get_async_order(order_id)
                if not data or not isinstance(data, dict):
                    time.sleep(2)
                    continue
                pc = data.get("position-changed")
                if not pc or (isinstance(pc, dict) and pc == {}):
                    time.sleep(2)
                    continue
                msg = pc.get("msg") if isinstance(pc, dict) else pc
                if isinstance(msg, dict) and msg.get("status") == "closed":
                    invest = float(msg.get("invest") or msg.get("buy_amount") or 0)
                    if msg.get("close_reason") == "expired":
                        close_profit = float(msg.get("close_profit") or 0)
                        profit = round(close_profit - invest, 2)
                    else:
                        profit = round(float(msg.get("pnl_realized") or 0), 2)
                    return (profit,)
            except (KeyError, TypeError, AttributeError, IndexError) as e:
                logging.debug("poll_digital_result: %s", e)
            time.sleep(2)
        return None
    ...
    def has_open_binary_turbo_positions(self):
        """Retorna True se existir ao menos uma posição aberta em turbo, binary ou multi-option (evitar múltiplas entradas)."""
        for instrument_type in ("turbo-option", "binary-option", "multi-option"):
            try:
                ok, msg = self.get_positions(instrument_type)
                if not ok or msg is None:
                    continue
                if isinstance(msg, list) and len(msg) > 0:
                    return True
                if isinstance(msg, dict):
                    pos = msg.get("positions") or msg.get("data") or msg.get("positions_data") or []
                    if isinstance(pos, list) and len(pos) > 0:
                        return True
            except Exception as e:
                logging.debug("has_open_binary_turbo_positions %s: %s", instrument_type, e)
        return False

    def has_open_digital_positions(self):
        """Retorna True se existir ao menos uma posição aberta em digital-option."""
        try:
            ok, msg = self.get_positions("digital-option")
            if not ok or msg is None:
                return False
            if isinstance(msg, list) and len(msg) > 0:
                return True
            ...
            logging.debug("has_open_digital_positions: %s", e)
        return False
    ...
    def buy_digital_spot_v2(self, active, amount, action, duration):
        ...
        while self.api.digital_option_placed_id.get(request_id) is None:
            if time.time() - start_t >= 10:
                logging.error("buy_digital_spot_v2 timeout ao aguardar digital_option_placed_id (req_id=%s)", request_id)
                return False, "timeout_digital_option_placed_id"
