#!/usr/bin/env python3
"""Adiciona coluna url na tabela pwa_custom_messages se não existir. Rode uma vez: python scripts/migrate_pwa_custom_message_url.py"""
...
def main():
    ...
    with engine.connect() as conn:
        try:
            conn.execute(text("ALTER TABLE pwa_custom_messages ADD COLUMN url VARCHAR(500)"))
            conn.commit()
            print("Coluna url adicionada em pwa_custom_messages.")
        except Exception as e:
            msg = str(e).lower()
            if "already exists" in msg or "duplicate_column" in msg:
                print("Coluna url já existe.")
            else:
                print("Erro:", e)
                sys.exit(1)
